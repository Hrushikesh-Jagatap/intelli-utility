...
const config = {
  baseURL: `${LOAD_BALANCER}/lms/apis`,
  // timeout: 5000,
};
...
const getDeliveryInformationViaId = async ({ headers, params = null }) => {
  try {
    const options = {
      headers,
    };
    const { userId } = params;
    const { data } = await apiService.get(`/v1/profile/${userId}`, options);
    return data;
  } catch (error) {
    throw new INTERNAL_SERVER_ERROR(error);
  }
